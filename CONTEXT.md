...
`--yolo` auto-accept, and every doc status flip (schema and rules: the
colony `Audit log` section). Provenance, not state: it is never read for
routing or resume; the artifacts remain the state.
