...
format, per the template at `docs/templates/adr.md`. They are written by the
architect agent during `/waggle` and accepted only by the human.
...
- **accepted** — set **only by the human** during `/waggle`; never
...
1. Write a **new** ADR (next free `ADR-NNNN`) through the normal `/waggle`
