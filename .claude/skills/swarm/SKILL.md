...
- All durable state lives in GitHub. /swarm is **idempotent**: after an
...
2. **Deterministic gate**: the milestone `.description` must contain the
   ...
   /comb first) and stop. Never proceed on a missing marker, never ask
...
2. **Discover the epic**: filter for issues that carry the `hive:managed`
   ...
   /comb?) or multiple matches (corrupted milestone). Capture its number
   as `epic#`.
...
2. Spawn the **guard** agent (Agent tool) with: the issue number, the full
...
2. Edit the PRD (path from Step 0.3): set frontmatter
   ...
   points (the other is /comb materialization).
3. Commit and push:
...
- Your only memory is the running `issue# → 1-line summary` table and the
  ...
  what makes re-running /swarm after any interruption safe.
