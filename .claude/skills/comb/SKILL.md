---
name: comb
description: "Turn an approved PRD into a reviewed plan.yaml and materialize it as a GitHub milestone with an epic and a task DAG. Invoke explicitly as /comb <PRD-id> (e.g. /comb PRD-003). Runs planner → three parallel plan reviewers → human approval gate → idempotent materialization."
disable-model-invocation: true
---

# /comb — plan an approved PRD
...
materialize it into GitHub issues. Load the **gh-conventions** skill for
...
2. Spawn the **planner** agent (Agent tool) with: the PRD path, the list of
...
3. Extract that YAML block and write it to
   ...
   `https://github.com/beelieve-ai/hive/blob/main/...` URLs). If the
...
1. **Round 1: spawn ALL THREE reviewers IN PARALLEL — three Agent tool
   calls in ONE message**: `plan-reviewer-context`, `plan-reviewer-dag`,
   `plan-reviewer-sizing`. Each gets exactly one input: the plan.yaml
   path.
...
`https://github.com/beelieve-ai/hive/blob/main/...` URLs — those links
404 unless the docs are on `main` first. So, before the first
`gh issue create`:
...
`/swarm`'s epic discovery filters on both; an epic missing either is
...
created; re-running `/comb $ARGUMENTS` resumes here without duplicates.
...
This marker is `/swarm`'s deterministic start gate — without it the
