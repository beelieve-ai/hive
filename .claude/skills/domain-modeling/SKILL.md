...
description: Sharpen fuzzy language into a canonical project vocabulary and maintain the root CONTEXT.md glossary. Use during grilling sessions (/pollinate, /sting) whenever a term is vague, overloaded, or contested, or whenever the glossary needs a new or corrected entry.
---
...
- **Architecture decisions are not glossary content.** When a resolved
  question turns out to be a decision (not a term), route it to `/waggle`,
