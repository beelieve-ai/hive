---
name: waggle
description: Drive architecture decisions for a PRD into MADR 4.0 ADRs — worthiness-test candidates, spawn one architect per passing decision, present options, and record human acceptance. Invoke as /waggle <PRD-id> [topic], e.g. `/waggle PRD-003` or `/waggle PRD-003 queue backend` to focus on one topic.
disable-model-invocation: true
---

# /waggle — architecture decisions for a PRD

You are the orchestrator. Read the `writing-adrs` and `crosslinking` skills
...
Spawn **one `architect` agent per passing decision** with the Agent tool.
...
- Verify all MADR sections from `docs/templates/adr.md` are present with
...
is a **new** `/waggle` run that supersedes it — never an edit.
...
command produces documents only. (`/comb` pushes docs before materializing.)
...
was recorded, and any superseded ADRs. Suggest `/comb <PRD-id>` as the next
